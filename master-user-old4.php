        ...
        <div class="box box-primary">
          ...
          <div class="box-body">
            ...
            <div class="table-responsive no-padding">
              <table class="table">
                <?php
                $arr_jumlah_rowspan = array();
                $sql =
                "SELECT
                  a.id,
                  count(b.nama) as jumlah_rowspan
                FROM
                  tb_master_navigasi AS a
                LEFT JOIN
                  tb_master_navigasi_detail AS b ON (b.id = a.id)";
                $sql .= "GROUP BY a.nama";
                $res = mysqli_query($db,$sql) OR die('error 529');
                while($row = mysqli_fetch_assoc($res))
                {
                  $arr_jumlah_rowspan[$row['id']] = $row['jumlah_rowspan'];
                }
                
                $sql =
                "SELECT
                  *,                  
                  a.nama AS navigasi_utama,
                  b.nama as navigasi_sub
                FROM
                  tb_master_navigasi AS a
                LEFT JOIN
                  tb_master_navigasi_detail AS b ON (b.id = a.id)";
                $sql .= "ORDER BY a.posisi,b.posisi";
                $res = mysqli_query($db,$sql) OR die('error 529');
                if(mysqli_num_rows($res) == 0)
                {
                ?>
                ...
                <tbody>
                  ...
                  while($row = mysqli_fetch_assoc($res))
                  {      
                    $navigasi_cetak = $row['navigasi_utama'];                      
                    if($navigasi_cetak == $navigasi_utama)
                    {
                      $navigasi_cetak = '-';                   
                    }  
                    else
                    ...
                    ?>
                  <tr style="<?php if(isset($_GET['id']) AND $row['id']==$_GET['id']) echo 'background-color:darkorange'; else if ($nomor % 2 == 0) { echo $background_ganjil; } else { echo $background_genap; }?>" class="tr-hover">
                    <td rowspan="<?php echo $arr_jumlah_rowspan[$row['id']]; ?>"><?php echo $navigasi_cetak; ?></td>
                    <td><?php echo $nomor++; ?></td>
                    <td><?php echo $row['navigasi_sub']; ?></td>
                  </tr>
                  <?php
                    }   
                    else
                  ...
                  <?php
                    ...
                    $navigasi_utama = $row['navigasi_utama'];
                  }
                  ?>
                </tbody>
                <?php 
